...
contract betshop { //betshop  
    ...
    mapping(uint256 => buyer)public bs;  //구매자 상태추적
    ...
    struct Meta {
     ...
     struct buyer {
        uint256 pay; // 결제금액 bet
        uint256 pid;  //구매한 제품id
        address owner; // 구매자 어카운트
    ...
    function newMeta(string memory _name,string memory _detail, string memory _img, uint256 _price,uint256 _left,uint256 _rate,uint256 _butreward) public onlyStaff(5) {
    ...
    function buy(uint _pid,uint num,string memory name,string memory house,string memory phone) public {  
        ...
        require(g4() >=  metainfo[_pid].butreward * num, "but not enough");
        ...
        but.transfer(msg.sender,num* metainfo[_pid].butreward);
    ...
    function g5(uint _bid) public view virtual returns (address) {  //구매자 주소
