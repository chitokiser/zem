document.addEventListener("DOMContentLoaded", function () {
    let metaddr = {
        metmarket: "0x3BB2f4C7fB4ecEc9a24a39264e882a09D5A70D28" //tanza
    };
    ...
    let TtopSync = async () => {  
        ...
        let icut = await meta5Contract.g5();   //계약잔고
        ...
        document.getElementById("Cutbal").innerHTML = icut;  // 누적 매출
    };
    ...
    async function displayMetaInfo() {
        try {
            ...
            for (let i = 0; i < ipid; i++) {
                try {
                    const tanzaImage = `..\\images\\tanza\\${i}.png`;
                    const metaInfo = await getMetaInfoByNum(meta5Contract, i);
                    const infoHtml = `
                   <img src="${tanzaImage}" alt="Lady ${i + 1}" />
...
<p class="card-text"><strong>BUT Number Provided:</strong> ${metaInfo.info4}BUT</p>
 ...
 <button type="button" onclick="submitPurchaseForm(${i})">Input complete</button>
 <button type="button" onclick="closePurchaseForm(${i})">Cancel</button>
</form>
 </div>
 </div>
 </div>`;
